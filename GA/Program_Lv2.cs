...
namespace 알고리즘
{
    class Program
    {
        ...
        public static dayVaccinatingList[] Mutation(wrapper[] topTeer, int endDay)
        {
            ...
            childrenList = new dayVaccinatingList[4];

            for (int i = 0; i < topTeer.Length; i++)
            {

                childrenList[0].a[0].howOldAreYou[0] = topTeer[i].d.a[0].howOldAreYou[1];           // 첫 번째(0번째) 날의 청장년 배분률을 미성년 배분률로
                childrenList[0].a[0].howOldAreYou[1] = topTeer[i].d.a[0].howOldAreYou[2];           // 첫 번째(0번째) 날의 노인 배분률을 청장년 배분률로
                childrenList[0].a[0].howOldAreYou[2] = topTeer[i].d.a[0].howOldAreYou[0];           // 첫 번째(0번째) 날의 미성년 배분률을 노인 배분률로
            }
            return childrenList;
        }
